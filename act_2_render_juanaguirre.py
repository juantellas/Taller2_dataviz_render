...
#file_path = "https://drive.google.com/uc?id=1--BCoussw9YtNSGpxq8r1zQbjorXuFNI"

file_id = "1--BCoussw9YtNSGpxq8r1zQbjorXuFNI"
file_path = f"https://drive.google.com/uc?id={file_id}"

shp_path = "shapefiles\MGN_DPTO_POLITICO.shp"

df = pd.read_csv(file_path, sep=None, engine="python", encoding="latin-1")
gdf = gpd.read_file(shp_path)

df.columns = df.columns.str.strip().str.replace('\n', '').str.replace('\r', '').str.replace(' ', '_').str.upper()
...
df_dept = df.groupby('DEPARTAMENTO_DE_OFERTA_DEL_PROGRAMA', as_index=False).agg(
    CANTIDAD_PROGRAMAS=('PROGRAMA_ACADÉMICO', 'count'),
    TOTAL_MATRICULADOS=('TOTAL_MATRICULADOS', 'sum'),
    NIVELES_DISTINTOS=('ID_NIVEL', 'nunique'),
    AREAS_DISTINTAS=('ID_AREA', 'nunique'),
    NUCLEOS_DISTINTOS=('ID_NUCLEO', 'nunique')
)

df_dept['PROMEDIO_MATRICULADOS'] = (
    df_dept['TOTAL_MATRICULADOS'] / df_dept['CANTIDAD_PROGRAMAS']
)
